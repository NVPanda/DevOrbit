    ...
    def post(self):
        ...
        try:
            ...
            response_data = {
                ...
                "post": post_content,
                "data": data_atual,
                "img_url": f"http://127.0.0.1:5000/files/{os.path.basename(img_path)}" if img_path else "https://cdn-icons-png.flaticon.com/512/847/847969.png"
            }
...
class UploadBanner(Resource):
    def post(self, user_id):
        ...
        cursor.execute("UPDATE usuarios SET banner = ? WHERE id = ?", (relative_file_path, user_id))
        conn.commit()
        conn.close()
